...
# Financial worth
clams <- combined_data %>% 
  filter(species_group == "Shellfish", species_name =="Manilla Clam", Gear.category =="Dredges", port_nationality == "UK - England") %>% 
  mutate(worth_1 = Value...000s./landed_weight_tonnes,
         ) %>% 
  group_by(year) %>%  
  summarise(mean_worth_1000GPB_per_t = mean(worth_1, na.rm = TRUE),
            sd_worth_1000GPB_per_t = sd(worth_1, na.rm = TRUE))

  
  
razors <- combined_data %>% filter(port_nationality == "UK - England", species_group == "Shellfish", species_code  == "RAZ") %>% 
  mutate(worth_1 = Value...000s./landed_weight_tonnes,
  ) %>% 
  group_by(year) %>% 
  summarise(mean_worth_1000GPB_per_t = mean(worth_1, na.rm = TRUE),
            sd_worth_1000GPB_per_t = sd(worth_1, na.rm = TRUE),
            se_worth_1000GPB_per_t = plotrix::std.error(worth_1, na.rm = TRUE))
